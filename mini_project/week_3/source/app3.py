...
products = [
    ...
    "Chopped salad",
    "Pasta House salad",
    "Special spicy chicken & avocado wraps",
    "Hot chocolate",
    "Mocha",
    "coffee",
    "Americano",
    ...
    while True:
        ...
        # product menu
            ...
            # update product
                ...
                user_input_index = input(
                    "Enter the number of the product you would like to update:\n"
                )
                user_input_product_name = input(
                    "What product would you like to add instead?\n"
                )
        ...
        # couriers menu
            ...
            if input_courier == "0":
                ...
                couriers_index_list()
                input_index_courier_update = input(
                    "Enter the number of the courier you would like to update:\n"
                )
                input_updated_courier = input(
                    "What courier would you like to add instead?\n"
                )

                couriers[
                    int(input_index_courier_update)
                ] = input_updated_courier.title()

            elif input_courier == "4":
                couriers_index_list()
                input_delete_courier = input(
            ...
            # get user input to update existing order
                ...
                while True:
                    print(
                        "Please ensure to enter a valid phone number and no blank spaces."
                    )
                    user_input_phone_num = input("Enter customer phone number:\n")
                    if (
                        user_input_phone_num.isdigit()
                        and len(user_input_phone_num) == 11
                        or len(user_input_phone_num) == 10
                    ):
                        break
            ...
            # printed order status list with index
                ...
                input_status_index = input(
                    "What would you like to update the order status to?\n"
                )

                chosen_order.update(
                    {"status": app2.order_status_list[int(input_status_index)]}
                )
            ...
            # get user input to update order
                ...
                while True:
                    if (
                        input_update_phone == ""
                        or input_update_phone.isdigit()
                        and len(input_update_phone) == 11
                        or len(input_update_phone) == 10
                    ):
                        break
                    else:
                        print("Please ensure to enter a valid phone number.")
                        input_update_phone = input(
                            "Enter updated customer phone number:\n"
                        )

                if (
                    input_update_name
                    or input_update_address
                    or input_update_phone == ""
                ):
                    print(
                        f"No update was conducted. Order information will remain the same.\n {chosen_order_property_update}"
                    )
                    continue
                else:
                    chosen_order_property_update.update(
                        {
                            "customer_name": input_update_name,
                            "customer_address": input_update_address,
                            "customer_phone": input_update_phone,
                        }
                    )
                print(
                    f"Order properties update successful!\n {chosen_order_property_update}"
                )
            elif input_orders_menu == "5":
                orders_index_list()

                input_delete_order_index = input(
